...
const Header = () => {
  ...
  const navLinkClass = (href: string) =>
    `text-base font-medium transition-colors ${
      activeHash === href ? "text-brand-text" : "text-ink-700 hover:text-brand-text"
    }`;
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 px-4 pt-4 sm:px-6 sm:pt-5">
      ...
      <div className="relative mx-auto w-full max-w-8xl rounded-2xl border border-border bg-surface shadow-lg">
        <div className="flex items-center justify-between gap-4 px-5 py-3 sm:px-6 sm:py-4">
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              className={`bg-brand text-ink-900 px-4 py-2 sm:px-5 sm:py-2.5 rounded-pill hover:bg-brand-600 font-semibold text-sm whitespace-nowrap ${pressable}`}
            >
        ...
        {navbarOpen && (
          <div className="lg:hidden absolute inset-x-0 top-full mt-3 rounded-2xl border border-border bg-surface shadow-lg p-5">
